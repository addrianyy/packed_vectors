...
macro_rules! impl_operator {
    (
        $name: ident,
        $op: ident, $op_function: ident,
        $op_assign: ident, $op_assign_function: ident,
        $function: item
    ) => {
        ...
        impl ops::$op_assign for $name {
            #[inline(always)]
            fn $op_assign_function(&mut self, rhs: Self) {
...
macro_rules! make_vector_type {
    ($name: ident, $type: ty, $lanes: expr) => {
        ...
        impl_operator! { $name, BitAnd, bitand, BitAndAssign, bitand_assign,
            fn bitand(self, rhs: Self) -> Self::Output {
                unsafe { Self(_mm256_and_si256(self.0, rhs.0)) }
            }
        }

        impl_operator! { $name, BitOr, bitor, BitOrAssign, bitor_assign,
            fn bitor(self, rhs: Self) -> Self::Output {
                unsafe { Self(_mm256_or_si256(self.0, rhs.0)) }
            }
        }

        impl_operator! { $name, BitXor, bitxor, BitXorAssign, bitxor_assign,
...
macro_rules! impl_basic_operations {
    ...
    (
        ...
        impl_operator! {$name, Add, add, AddAssign, add_assign,
            fn add(self, rhs: Self) -> Self::Output {
                unsafe { Self($add(self.0, rhs.0)) }
            }
        }

        impl_operator! {$name, Sub, sub, SubAssign, sub_assign,
...
impl_operator! { Int32x8, Mul, mul, MulAssign, mul_assign, 
    fn mul(self, rhs: Self) -> Self {
        unsafe { Self(_mm256_mul_epi32(self.0, rhs.0)) }
    }
}

impl_operator! { Uint32x8, Mul, mul, MulAssign, mul_assign, 
    fn mul(self, rhs: Self) -> Self {
